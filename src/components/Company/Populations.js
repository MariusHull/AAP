...
export default class Sites extends Component {
    ...
    handleSubmit = () => {
        ...
        axios
          .post(`http://localhost:3001/api/companies/population/${localStorage.getItem("companyId")}`, {populationName, siteId: this.props.match.params.index})
          .then(r => {
              console.log(r.data)
    ...
    componentDidMount() {
        ...
        axios
          ...
          .then(r => {
            ...
            this.setState({ populations: r.data.sites[this.props.match.params.index].populations });
          });
      }
  ...
  render() {
    ...
    return (
      <Container style={{ width: "100%" }}>
        <NavBar />
        <Message info style={{ width: "23%", margin:'10px auto'}} header={"Gestions de vos populations"} content={"Sur cette page vous pouvez consulter vos populations et en créer de nouveaux en cliquant sur '+'"}/>
        <Container>
        {this.state.populations.map((e, i) => (
                <Card centered href={`/survey/${this.props.match.params.index},${i}`}>
                    <Card.Content>
                    <Card.Header>
                    {e.name}
                    </Card.Header>
        ...
        onClose={this.handleClose} style={{width:'30%', marginTop:'5%'}}>
            ...
            <Form onSubmit={this.handleSubmit}>
                <Form.Group>
                    <Form.Input placeholder='Name' name='populationName' value={populationName} onChange={this.handleChange} />
